...
/* apply( tpl, arg ) -- computes 'tpl' in singleton mode with current
   value generated by arg as current input and returns the result */

dojox.jtlc._declareTag( 'apply', {
	constructor: function( tpl, arg ) {
		if( !tpl || !arg )	throw Error( "apply() requires exactly 2 arguments" );
		this.func = tpl;
		this.arg = arg;
	},
	...
	compile: function( self ) {
		this.compile( self.arg );
		this.nonAccumulated( function(){ this.compile( self.func ); }, this.popExpression() );
	}
} );
...
/* recurse( [tpl] ) -- walks the tree generating all objects in sequence */

dojox.jtlc._getDescendants = function( next ) {
	var stack = [], result = [];
	for(;;) {
		for( var i in next )
...
dojox.jtlc._declareTag( 'recurse', dojo.declare( dojox.jtlc._ArrayOrGenerator, {
	compile: function( self ) {
		self.compileImpl.call( this, self, this._descendantGenerator = this._descendantGenerator || this.addGlobal( dojox.jtlc._getDescendants ) );
	}
} ) );

/* sort( key [, arg] ) -- sorts the input treated as a singleton array. Key is either
   a singleton template or an array of such; use desc() wrapper to specify descending
   order on a specific key component */

dojox.jtlc._declareTag( 'desc', {

	constructor: function( tpl ) {
		if( !tpl )	throw Error( "desc() must have an argument" );
		this.arg = tpl;
	},
...
dojox.jtlc._declareTag( 'sort', {

	constructor: function( key, arg ) {
		this.key = !key || key instanceof Array ? key : [ key ];
		this.arg = arg;
	},
	...
	compile: function( self ) {
		...
		var a = this.popExpression();
		if( a.length > 2 ) {
			var v = this.addLocal();
			if( v!=a )	this.code.push( v + '=' + a + ';' );
			a = v;
		}

		if( self.key ) {
			this.code.push( a + '.sort(function($A,$B){var $a,$b;' );

			for( var i=0; i<self.key.length; ++i ) {
				this.nonAccumulated( 
					function() { this.compile( self.key[i] ); },
					'$A'
				);
				this.code.push( '$a=' + this.popExpression() + ';' );
				this.nonAccumulated( 
					function() { this.compile( self.key[i] ); },
					'$B'
				);
				this.code.push( '$b=' + this.popExpression() + ';' );

				this.code.push( 'if($a<$b)return ' + ( self.key[i] instanceof dojox.jtlc.tags._desc ? '1;' : '-1;' ) );
				this.code.push( 'if($b<$a)return ' + ( self.key[i] instanceof dojox.jtlc.tags._desc ? '-1;' : '1;' ) );
			}

			this.code.push( 'return 0;});' );
		} else this.code.push( a + '.sort();' );
	...
	return function( key, arg ) {
		return dojox.jtlc.tags.from( new cls( key, arg ) );
	}
} );
...
/* filter( [condition, [tpl]] ) -- returns only the elements that satisfy
   'condition' -- if omitted, only the truthy elements */

dojox.jtlc._declareTag( 'filter', {
	
	constructor: function( condition, tpl ) {
	...
	compile: function( self ) {
		if( !this.loop || this.loop.started() )
			throw Error( "filter() is never a singleton, wrap it in many() or []" );
		this.compile( self.arg );
		var	value = this.popExpression(),
			t = this.addLocal();
		if( t != value )	this.code.push( t + '=' + value + ';' );
		...
		if( self.condition ) {
			this.nonAccumulated( function() { this.compile( self.condition ); }, t );
			cond = this.popExpression();
		}

		this.code.push( 'if(' + cond + ')continue;' );
		this.expressions.push( t );
	}
} );
...
dojox.jtlc.tags.defined = function( tpl ) {
	return dojox.jtlc.tags.filter( 
		dojox.jtlc.tags.expr( "typeof $==='undefined'" ), tpl
	);
}
